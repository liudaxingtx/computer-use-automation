...
The first four target the local mock (`localhost:9000`); the last three target **real, publicly-deployed test sites** — proving the same engine drives live sites, not just the mock.
...
| `saucedemo_checkout` | saucedemo.com | Login → add to cart → checkout → finish an order (real site, 11 steps) | `user-name`, `password`, `firstName`, `lastName`, `postalCode` |
| `theinternet_login` | the-internet.herokuapp.com | Log into the Secure Area (real site) | `username`, `password` |

`saucedemo_checkout` is the headline multi-step case: a full transactional flow (login → add-to-cart → cart → shipping form → order overview → confirmation) discovered, recorded, and replayed against a live React storefront — including disambiguating its six identical "Add to cart" buttons and waiting out its client-side renders.
...
- **Repeated-control disambiguation.** A real storefront has six identical "Add to cart" buttons. Each locator records a `name_ordinal` (its position *within* the role+name group) so replay can target "the 1st Add to cart" deterministically — `get_by_role(name=…)` alone would hit a strict-mode violation on six matches.
- **Reasoning-model output drift.** `deepseek-v4-pro` emits a `reasoning_content` monologue *plus* `content`; on complex pages the monologue can consume the whole output budget and leave `content` empty. We give it a generous budget and fall back to parsing the JSON out of `reasoning_content`, with a short backoff retry — so a transient drift self-heals instead of failing a discovery.
