...
- **One registry for every long-running step.** The same background-job mechanism also runs new-task **discovery**: submit, close the modal immediately, and a toast + result modal (verification result, screenshot, close / view-task buttons) arrives when the flow is recorded and verified. Discovery is just another job type in the lock-guarded registry, not a special-case path.
